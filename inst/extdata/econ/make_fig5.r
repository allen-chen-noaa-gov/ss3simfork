...
dirsin <- c("112321_mortup9",
    "012722_mortup9_nosurvey", 
    "031422_mortup8_random_20",
    "031422_mortup8_trend_20")

linegraphsave <- NULL
for (i in 1:4) {

#rawoutputdir for files (iterations and scenarios)
rawoutputdir <- paste0("C:\\Users\\Allen.Chen\\SS3SIM_SCRATCH\\for_paper\\",
    dirsin[i])
...
linegraph = aggregate(totabundout$RelDiff, 
    list(Year = totabundout$year, Index = totabundout$Index), median, na.rm=TRUE)
...
totabundtemp <- reshape(data = totabundout[c("year", "iter", "Index", "RelDiff")],
        idvar = c("year", "iter"),
        v.names = c("RelDiff"),
        timevar = "Index",
        direction = "wide")
...
totabundtemp[c("RelDiff.Randomly sampled")] <- totabundtemp[c("RelDiff.Randomly sampled")] - 
    totabundtemp$RelDiff.True
...
linegraph = aggregate(totabundout$RelDiff,
    list(Year = totabundout$year, Index = totabundout$Index), mean, na.rm = TRUE)
names(linegraph)[names(linegraph) == 'x'] = 'RelDiff'
...
lineout = ggplot(data = linegraph,
        ...
        geom_point(aes(shape=Index), size=2, fill="white", stroke = 0.75,
            position=pd) +
        geom_ribbon(aes(x=Year, ymin = RelDiff-(2*se), ymax = RelDiff+(2*se), fill=Index, alpha = Index)) +
...
lineout = ggplot(data = linegraphsave,
        ...
        geom_point(aes(shape=Index), size=3, fill="white", stroke = 1.2,
            position=pd) +
        geom_ribbon(aes(x=Year, ymin = RelDiff-(2*se), ymax = RelDiff+(2*se), 
